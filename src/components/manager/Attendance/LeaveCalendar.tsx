...
Location: ${event.location ? `${event.location.lat.toFixed(4)}, ${event.location.lng.toFixed(4)}` : 'N/A'}`;
    }
  ...
  const renderCalendar = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push(
        <div 
          key={d} 
          className={`calendar-day rounded-xl bg-white shadow-neomorphic text-center py-2 cursor-pointer transition-all duration-150 hover:bg-[#e9eef5] border border-transparent ${todayClass ? 'border-blue-400' : ''} ${isSelected ? 'ring-2 ring-blue-400' : ''}`}
  ...
  return (
    <div className="neo-box bg-[#f4f7fa] rounded-2xl shadow-neomorphic p-4 mb-4 max-w-xl mx-auto font-sans">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-bold text-gray-800">Attendance & Leave Calendar</h3>
        <div className="flex items-center gap-2">
          <button className="neo-button rounded-lg shadow-neomorphic px-2 py-1" onClick={prevMonth}>
            <i className="bi bi-chevron-left text-lg"></i>
          </button>
          <span className="font-semibold text-base text-gray-700 px-2">
            {currentMonth.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
          </span>
          <button className="neo-button rounded-lg shadow-neomorphic px-2 py-1" onClick={nextMonth}>
      ...
      <div className="grid grid-cols-7 gap-2 mb-2">
        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, index) => (
          <div key={index} className="text-center text-gray-500 font-semibold text-sm tracking-wide">
            {day}
          </div>
        ))}
      </div>
