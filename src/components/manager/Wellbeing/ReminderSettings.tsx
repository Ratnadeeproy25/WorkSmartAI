...
const ReminderSettingsComponent: React.FC<ReminderSettingsProps> = ({ 
  ...
  const updateTeamWellbeingSettings = (teamWellbeingSettings: Partial<ReminderSettings['teamWellbeing']>) => {
    updateSettings({
      teamWellbeing: { ...settings.teamWellbeing, ...teamWellbeingSettings }
    });
  };
    ...
    <div className="p-6 bg-[#e0e5ec] rounded-xl shadow-[10px_10px_20px_#a3b1c6,_-10px_-10px_20px_#ffffff] mb-8">
      ...
      {isOpen && (
        <div className="mt-6 space-y-6">
          ...
          {/* Team Wellbeing Reminders */}
          <div className="p-4 bg-[#e0e5ec] rounded-lg shadow-[inset_5px_5px_10px_#a3b1c6,_inset_-5px_-5px_10px_#ffffff]">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-700">Team Wellbeing Check-in Reminders</h3>
              <label className="flex items-center cursor-pointer">
                <input 
                  type="checkbox" 
                  className="hidden" 
                  checked={settings.teamWellbeing?.enabled ?? false}
                  onChange={e => updateTeamWellbeingSettings({ enabled: e.target.checked })}
                />
                <div className={`w-12 h-6 rounded-full relative p-1 transition-colors duration-300 ease-in-out ${settings.teamWellbeing?.enabled ? 'bg-green-500' : 'bg-gray-300'}`}>
                  <div className={`bg-white w-4 h-4 rounded-full shadow transform transition-transform duration-300 ease-in-out ${settings.teamWellbeing?.enabled ? 'translate-x-6' : 'translate-x-0'}`}></div>
                </div>
              </label>
            </div>
            
            {settings.teamWellbeing?.enabled && (
              <div className="space-y-4">
                <div>
                  <label className="block text-gray-600 mb-2">Frequency:</label>
                  <select 
                    value={settings.teamWellbeing.frequency}
                    onChange={e => updateTeamWellbeingSettings({ frequency: e.target.value as any })}
                    className="w-full bg-[#e0e5ec] rounded-lg p-3 shadow-[inset_5px_5px_10px_#a3b1c6,_inset_-5px_-5px_10px_#ffffff] transition-all duration-300 outline-none border-none"
                  >
                    <option value="weekly">Weekly</option>
                ...
                <div>
                  <label className="block text-gray-600 mb-2">Day of the week:</label>
                  <select 
                    value={settings.teamWellbeing.day}
                    onChange={e => updateTeamWellbeingSettings({ day: Number(e.target.value) })}
                    className="w-full bg-[#e0e5ec] rounded-lg p-3 shadow-[inset_5px_5px_10px_#a3b1c6,_inset_-5px_-5px_10px_#ffffff] transition-all duration-300 outline-none border-none"
                  >
                ...
                <div>
                  ...
                  <input 
                    type="time" 
                    value={settings.teamWellbeing.time || "10:00"}
                    onChange={e => updateTeamWellbeingSettings({ time: e.target.value })}
                    className="w-full bg-[#e0e5ec] rounded-lg p-3 shadow-[inset_5px_5px_10px_#a3b1c6,_inset_-5px_-5px_10px_#ffffff] transition-all duration-300 outline-none border-none"
                  />
                </div>
                ...
                <div className="mt-2">
                  <div className="text-sm text-gray-500 italic">
                    You'll be reminded to check in on your team's wellbeing on the selected day and time.
                  </div>
                </div>
              </div>
            )}
