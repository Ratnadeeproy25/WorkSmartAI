...
const PerformanceChart: React.FC<PerformanceChartProps> = ({ className, chartData }) => {
  const [timeRange, setTimeRange] = useState<'week' | 'month' | 'quarter'>('week');
  
  // Default chart data if no API data is provided
  ...
  const data = chartData || {
    labels: defaultChartData[timeRange].labels,
    datasets: [
      {
        label: 'Productivity',
        data: defaultChartData[timeRange].productivity,
        borderColor: '#3b82f6',
        tension: 0.4,
        fill: true,
      ...
      {
        label: 'Engagement',
        data: defaultChartData[timeRange].engagement,
        borderColor: '#10b981',
        tension: 0.4,
        fill: true,
