...
class NotificationService {
  ...
  // Update just a subset of settings without reinitializing everything
  public updateSettings(newSettings: Partial<ReminderSettings>): void {
    // Get existing settings
    const currentSettings = this.getCurrentSettings();
    
    // Create updated settings by merging new settings into current ones
    const updatedSettings = this.mergeSettings(currentSettings, newSettings);
    
    // Reinitialize with updated settings
    this.init(updatedSettings);
  }
  
  // Helper method to get current settings from localStorage or use defaults
  private getCurrentSettings(): ReminderSettings {
    try {
      ...
      console.error('Error getting current settings', e);
    }
  ...
  private mergeSettings(current: ReminderSettings, updates: Partial<ReminderSettings>): ReminderSettings {
    const result = { ...current };
    
    // Update breaks settings
    if (updates.breaks) {
      result.breaks = { ...result.breaks, ...updates.breaks };
    }
    
    // Update mood settings
    if (updates.mood) {
      result.mood = { ...result.mood, ...updates.mood };
    }
    
    // Update activities settings
    if (updates.activities) {
      result.activities = { ...result.activities, ...updates.activities };
    }
    
    // Update teamWellbeing settings
    if (updates.teamWellbeing) {
      result.teamWellbeing = { ...result.teamWellbeing, ...updates.teamWellbeing };
    }
