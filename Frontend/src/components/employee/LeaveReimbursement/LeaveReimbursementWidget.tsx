...
import { LeaveRequest, ReimbursementRequest, LeaveBalance } from './types';
...
const LeaveReimbursementWidget: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const leave = await leaveService.getLeaveRequests();
        setLeaveRequests(leave);
        
        // Fetch reimbursement requests
        const reimbursements = await reimbursementService.getReimbursementRequests();
        setReimbursementRequests(reimbursements);
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-6">
        ...
        <div className="neo-box p-6">
          <div className="flex justify-between items-center mb-6">
            <div className="animate-pulse h-6 w-60 bg-gray-200 rounded"></div>
            <div className="animate-pulse h-10 w-28 bg-gray-200 rounded"></div>
          </div>
          <div className="animate-pulse space-y-4">
            <div className="h-20 bg-gray-200 rounded"></div>
  ...
  if (error) {
    return (
      <div className="mt-6 neo-box p-6 text-red-600">
        <p>{error}</p>
        <button 
          onClick={() => window.location.reload()}
          className="neo-button mt-4 p-2 text-sm"
        >
          Retry
        </button>
      </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-6">
      ...
      <div className="neo-box p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link to="/employee/leave" className="neo-button primary px-4 py-2">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {leaveBalances.map((balance, index) => (
            <div key={index} className="neo-box p-4 text-center">
              <div className="text-2xl font-bold" style={{ color: balance.color }}>{balance.remaining}</div>
        ...
        <div className="mt-4 text-center">
          <Link to="/employee/leave" className="neo-button p-2 inline-flex items-center gap-2">
