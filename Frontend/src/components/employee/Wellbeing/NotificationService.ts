...
class NotificationService {
  ...
  private constructor() {
    // Initialize with timestamps from API when possible, but don't block
  ...
  private async fetchTimestamps() {
    try {
      ...
      if (data && data.notificationTimestamps) {
        this.notificationTimestamps = {
          lastBreak: data.notificationTimestamps.lastBreak,
          lastMood: data.notificationTimestamps.lastMood,
          lastActivity: data.notificationTimestamps.lastActivity
        };
      }
    } catch (error) {
      console.error('Failed to fetch notification timestamps:', error);
    }
  }
  ...
  public updateTimestamp(type: keyof NotificationTimestamps): void {
    ...
    this.notificationTimestamps[type] = now;
    
    // No need to manually update localStorage, API calls
    // will handle this when actions are performed
  }

  // Get timestamps 
  private getTimestamps(): NotificationTimestamps {
    return this.notificationTimestamps;
  }
  ...
  public addEventListener(type: 'break' | 'mood' | 'activity', callback: Function): void {
  ...
  public removeEventListener(type: 'break' | 'mood' | 'activity', callback: Function): void {
    if (this.listeners[type]) {
  ...
  private notifyListeners(type: 'break' | 'mood' | 'activity'): void {
    if (this.listeners[type]) {
